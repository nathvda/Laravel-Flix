...
<div class="flex gap-8 snap-x snap-mandatory scroll-mt-4 py-4">
@foreach($profile->movies as $movie)
<a href="/see/{{$movie->id}}" class="snap-start">
    <div class="w-60">
    <img class="w-60" src="/media/thumbnails/{{$movie->thumbnail}}" class="rounded-md"/>
...
<h3 class="text-white font-bolder text-xl">{{$genre->name}}</h3>
<div class="flex gap-8 snap-x snap-mandatory scroll-mt-4 py-4">
